...
def _format_device_string(device: dict):
    ...
    return f"VID: {hex(vid)}, PID: {hex(pid)} -> {width}x{height}"
...
def choose_device(devices: list) -> dict | None:
    ...
    try:
        selected_device = int(input("\nSelect your device (1-{}): ".format(len(devices))))
        ...
        print("Please enter a number corresponding to a device.\n")
        return choose_device(devices)
...
def main():
    parser = argparse.ArgumentParser(description="Thermalright LCD Control device selector")
