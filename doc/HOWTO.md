...
- The CLI/service layer, which configures and drives the screen.
...
The last step is to add support for a new device.

To do that, you need to add a new device in `supported_devices.py` and, if needed, adjust the
screen resolution-specific config under `resources/config`.
...
Last step is to edit the YAML theme/config files directly and save your changes.
